...
class MirralismMigrationInstaller:
    ...
    def _load_config(self) -> Dict:
        ...
        default_config = {
            ...
            "dependencies": {"python": ">=3.9", "node": ">=18.0.0", "git": ">=2.30"},
            "migration_phases": [
                "backup",
                "dependency_check",
                "file_organization",
    ...
    def _log(self, message: str, level: str = "info") -> None:
        ...
        print(
            f"{'🔧' if level == 'info' else '⚠️' if level == 'warning' else '❌'} {message}"
        )
    ...
    def check_dependencies(self) -> bool:
        ...
        try:
            ...
            if python_version >= (3, 9):
                checks.append(
                    ("Python", True, f"{python_version.major}.{python_version.minor}")
                )
            else:
                checks.append(
                    (
                        "Python",
                        False,
                        f"要求: >=3.9, 現在: {python_version.major}.{python_version.minor}",
                    )
                )
        except Exception as e:
