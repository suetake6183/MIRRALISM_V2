...
def audit_directories():
    ...
    print("\n📊 統計情報:")
    print("   設計書指定ディレクトリ: {len(design_dirs)}個")
    print("   実際存在ディレクトリ: {len(actual_dirs)}個")
    print("   設計通り存在: {len(existing)}個")
    print("   不足ディレクトリ: {len(missing)}個")
    print("   設計外ディレクトリ: {len(extra)}個")
    print("   設計整合率: {len(existing)/len(design_dirs)*100:.1f}%")
    ...
    if critical_missing:
        print("   🔴 重大リスク（{len(critical_missing)}個）: コア機能・データ格納不能")
        for d in critical_missing:
            print("      - {d}")

    if important_missing:
        print("   🟡 中程度リスク（{len(important_missing)}個）: 機能配置不明確")
