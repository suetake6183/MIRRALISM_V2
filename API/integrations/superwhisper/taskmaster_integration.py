...
if __name__ == "__main__":
    ...
    if result["success"]:
        if result.get("execution_result", {}).get("success", False):
            ...
            print("操作: {exec_result['action']}")
            print("メッセージ: {exec_result['message']}")
            if "task_data" in exec_result:
                print("タスクタイトル: {exec_result['task_data']['title']}")
        else:
            print("アクション: {result.get('action', 'none')}")
            print("理由: {result.get('reason', 'N/A')}")
    else:
