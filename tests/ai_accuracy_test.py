...
try:
    # CI互換性を優先して試行
    from Core.PersonalityLearning.ci_compatibility import get_personality_learning_system
    PERSONALITY_LEARNING_AVAILABLE = True
    USE_CI_COMPATIBILITY = True
except ImportError:
    try:
        from Core.PersonalityLearning.integrated_system import MirralismPersonalityLearning
        PERSONALITY_LEARNING_AVAILABLE = True
...
class AIAccuracyTester:
    ...
    def run_real_test(self, target_accuracy: float = 0.95) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            accuracy = results["passed_tests"] / results["total_tests"] if results["total_tests"] > 0 else 0
    ...
    def save_results(self, results: Dict, output_file: str = "ai_accuracy_results.json"):
        """結果保存"""
...
def main():
    ...
    parser.add_argument("--target-accuracy", type=float, default=0.95,
                       help="目標精度 (デフォルト: 0.95)")
    parser.add_argument("--mock-only", action="store_true",
                       help="モックテストのみ実行")
    parser.add_argument("--output", default="ai_accuracy_results.json",
                       help="結果出力ファイル")
