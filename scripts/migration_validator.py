...
class MigrationValidator:
            ...
            if not db_file.exists():
                ...
                try:
                    ...
                    if not tables:
                        ...
                        expected_tables = ["personality_data", "analysis_results", "user_sessions"]
    ...
    def run_performance_tests(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                if execution_time > self.validation_config["thresholds"]["max_calculation_time"]:
    ...
    def save_validation_report(self, output_file: Optional[Path] = None) -> Path:
        ...
        if output_file is None:
            ...
            output_file = self.root_dir / ".mirralism" / f"validation_report_{timestamp}.json"
    ...
    def check_rollback_needed(self, validation_results: Dict) -> bool:
        ...
        for check in validation_results.get("checks", []):
            if check["status"] == "failed":
                # 重要なチェックの失敗数をカウント
                if check["name"] in ["file_structure_validation", "core_systems_validation"]:
                    critical_failures += 1
...
def main():
    ...
    if args.auto_rollback and not validation_passed:
        ...
        if rollback_system.suggest_rollback(validator.results):
            ...
            if backup_id:
                print(f"ロールバック実行: python scripts/migrate_installer.py rollback --backup-id {backup_id}")
